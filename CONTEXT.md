...
**Annotation**:
A note attached polymorphically to a timeline entry — a Communication Event today, a Dead-Air Period once that lands (`docs/adr/0009`), never a Game Event. System-generated Annotations have a null `user_id` and a `topic` (`game_state_alignment` on a Communication Event); the field is a nullable FK so coach- and player-authored Annotations extend the same entity once timeline management lands. See `docs/adr/0008-game-state-alignment.md` and its 2026-09-07 amendment.
