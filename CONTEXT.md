...
Where one Session Participant sits in the recording lifecycle for their Session: `needs_consent`, then `ready`, then `recording`, then `completed`. It never steps back on its own and never skips a status it was meant to pass through, except for the resets below. `completed` is reachable from all three of the others: it means "this Session is over for me" rather than "I finished recording" (see `docs/adr/0014-participation-lifecycle.md`). `recording` is the status a player controls during a run and the one that authorises everything: only a `recording` participant may upload, and only their stored recordings are transcribed and swept at completion. A player stops recording and drops back to `needs_consent`; leaving the Session, and the Session returning to `queuing`, reset a participant the same way. Every one of those resets discards that participant's stored recordings, so nothing outlives the status that authorised it (see `docs/adr/0013-recording-control-and-departure.md`). A player joins at `needs_consent` and reaches `ready` by giving Consent. A Coach joins at `ready` and never records, so a Coach stays `ready` for the length of the run and reaches `completed` when the Session does. Every consented player moves to `recording` when the Coach starts the Session. Session completion then moves every participant still in the Session to `completed` in one sweep — the Coach and a player who stopped early included, whether or not their own AOD and VOD were among those delivered. A participant who had already left is not swept: they were not in the Session when it ended, and `left_at` says so.
_Avoid_: state, stage
