...
One marked interval on a Session's Timeline, with a start and end offset in milliseconds. Three kinds are built: a **Communication Event** (an interval), a **Game Event** (a point, `start == end`), and a **Dead-Air Period** (an interval). Coach-authored review points may become a fourth. The `session_timeline` endpoint groups Communication Events under each participant (`data.participants[].timestamps[]`) and merges Game Events and Dead-Air Periods into one session-level `data.game_events[]` ordered by `start_ms`, a Game Event before a Dead-Air Period on a tie.
...
**Annotation**:
A note attached polymorphically to a timeline entry, a Communication Event or a Dead-Air Period, never a Game Event. System-generated Annotations have a null `user_id` and a `topic` (`game_state_alignment` on a Communication Event, `dead_air` on a Dead-Air Period); the field is a nullable FK so coach- and player-authored Annotations extend the same entity once timeline management lands. See `docs/adr/0008-game-state-alignment.md` and its 2026-09-07 amendment, and `docs/adr/0009-dead-air-detection.md`.
