...
**Session status**:
A Session's lifecycle state: `queuing`, then `in_progress`, then **Delivering** (`delivering`), then `processing`, or `cancelled` from `queuing` (before recording starts), `in_progress` or `delivering` (aborting a run already underway). `in_progress` and `delivering` also return to `queuing` whenever no Session Participant is left at `recording`, whether the players stopped recording or left, because a run nobody is recording is not a run; a Coach can also stop the team's recording outright, which ends the run but keeps the roster. Either way every active participant resets to the status their role starts at and the Session's stored recordings are discarded, so the whole team consents again and records again before the next start (see `docs/adr/0013-recording-control-and-departure.md`). A Coach starts the Session to move it from `queuing` to `in_progress`, allowed only once every present player has given Consent, and that transition is also when recording begins for every consented player. The per-participant recording lifecycle is a separate term (see **Session Participant status**). Each recording player delivers their own audio and video by calling `POST /sessions/{session}/recording` directly, independent of the coach; re-uploading replaces what that player already sent rather than adding to it (see `docs/adr/0012-per-player-recording-uploads.md`). A Coach ends the run by moving the Session to `delivering`, which tells every client to finish its recorder and upload. Players can still upload there but can no longer start or stop recording, and the Session cannot go back to `in_progress`. A Coach completes a `delivering` Session, and only a `delivering` one, with a `game_events` payload once at least one recording player has stored both an audio and a video recording; the completion call itself carries no files (see `docs/adr/0015-end-of-run-and-end-of-participation.md`). `started_at` records when the current run began: `start()` stamps it and the return to `queuing` clears it. Aborting an `in_progress` or `delivering` Session discards any recordings already delivered for it: recordings only survive a Session once it reaches `processing`, so cancelling mid-run leaves nothing behind, even after some players have uploaded.
...
Where one Session Participant sits in their Session: `needs_consent`, then `ready`, then `recording`, then **Ending** (`ending`), then `completed`. It never steps back on its own and never skips a status it was meant to pass through, except for the resets below. `ending` is reachable from the first three: it means "this Session is over for me, and its analysis is still running", and `completed` means "this Session is over for me, and its timeline is ready" (see `docs/adr/0014-participation-lifecycle.md` and `docs/adr/0015-end-of-run-and-end-of-participation.md`). `recording` is the status a player controls during a run and the one that authorises everything: only a `recording` participant may upload, and only their stored recordings are transcribed at completion. A player keeps `recording` while the Session is `delivering`. A player stops recording and drops back to `needs_consent`; leaving the Session, and the Session returning to `queuing`, reset a participant the same way. Every one of those resets discards that participant's stored recordings, so nothing outlives the status that authorised it (see `docs/adr/0013-recording-control-and-departure.md`). A player joins at `needs_consent` and reaches `ready` by giving Consent. A Coach joins at `ready` and never records. Every consented player moves to `recording` when the Coach starts the Session. Session completion moves every participant still in the Session to `ending` in one sweep, the Coach and a player who stopped early included, whether or not their own AOD and VOD were among those delivered. The move to `timeline_ready` sweeps them on to `completed`, and a re-analysis resets them to `ending` until the rebuilt timeline is ready. A participant who had already left is not swept: they were not in the Session when it ended, and `left_at` says so.
_Avoid_: state, stage

**Delivery state**:
Which of their own recordings a Session Participant has stored for the current run: an AOD, a VOD, both or neither, each with its file facts. It is part of the participant's representation and of every participant broadcast, so a Coach can see who has delivered before completing. Any discard clears it.
